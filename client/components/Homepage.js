...
export default class Homepage extends Component {
    constructor(props) {
        super(props);
        this.state = {movies: [{}], page: 1, numberMovies: 0};
    ...
    componentDidMount() {
        ...
        axios.get('http://localhost:5000/api/movies/date/20/0')
            .then(res => {
                ...
                this.setState({numberMovies: res.data.length});
            })
    ...
    render() {
        return (
            <Container>
                <Row>
                {this.state.numberMovies > 0 ? <h4 className="pageHeadline">Newest (20) Movie Entries</h4> : <h4 className="pageHeadline">There are no movies yet</h4>}
